...
def read_master_rows(workbook_path: Path, sheet_name: str = DEFAULT_SHEET) -> tuple[list[dict[str, str | None]], list[str]]:
    ...
    for excel_row_number, row in enumerate(rows_iter, start=2):
        ...
        for col in OPTIONAL_COLUMNS:
            values[col] = clean_cell(row[header_index[col]]) if col in header_index else None
...
def build_database(
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.executemany(
            """
            INSERT INTO members (
                person_id, display_name, title, first_name, middle_name, last_name, suffix,
                institution, primary_email, secondary_email, phone,
                address1, address2, city, state_province, postal_code, country,
                region, membership_status, original_membership_code,
                member_since, last_paid_year,
